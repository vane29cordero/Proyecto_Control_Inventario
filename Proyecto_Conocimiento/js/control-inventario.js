...
function calcularTotalInventario(listado) {
    ...
    listado.forEach(function(parte) {
        valorTotalInicial += calcularValorParte(parte);
    });
...
function registrarEntrada(idParte, cantidad) {
    ...
    var stockNuevo = parte.cantidad + cantidad;

    if (stockNuevo > parte.capacidad_maxima) {
        return false;
    }

    parte.cantidad = stockNuevo;
...
function calcularPorcentajeOcupacion(parte) {
    ...
    var capacidadPorcentaje = (parte.cantidad/parte.capacidad_maxima) * 100;

    // TODO: Retornar ese porcentaje como número.

    return capacidadPorcentaje;
